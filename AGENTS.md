...
- Four planning files with distinct roles, kept in sync with actual project state:
  ...
  - `PLAN.md` - the **what**: the structured mapping of open TODO tasks into work packages and milestones. Each package is a table (`Task | Effort | Provides`, effort S < M < L), packages ordered top-to-bottom by urgency, re-ranked against the backlog before each next step. The swarm/team milestones (M1-M3) use checkboxes. **Finished tasks are removed from `PLAN.md`** - they live as one-liners in `TODO.md` `## Done` and in detail in `CHANGELOG.md`.
