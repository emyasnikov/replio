...
6. Bundled plugins live in the repo `plugins/` dir (shipped as `replio.plugins.bundled`), so add a new bundled plugin there + to the `plugins` config default, never to the core
