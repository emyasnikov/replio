...
- `/connect` now tests the provider connection before saving - a `GET <base_url>/v1/models` probe via the new `OpenAICompatibleProvider.check_connection()`, which reuses a shared `_fetch_models()` helper (also backing `list_models()`, behavior unchanged). Broken values print the error and need an explicit `Save anyway? [y/N]` to persist; a success message notes when the configured model is missing from the model list
- `/provider <name>` switches then probes the new connection and prints a warning when it fails (`Run /connect to fix`). Both probes are gated by the new `connect_check` config (default `true`; `false` skips them for offline/flaky networks)
...
- Tests - `check_connection` coverage in `tests/test_providers.py` (loopback success/empty/model-note/HTTP, network error, `list_models` unchanged) and `tests/test_engine.py` (resolution, override precedence, no state mutation, base-URL detection); `/connect` probe-before-commit (decline keeps config, accept saves, `connect_check: false` skips) and `/provider` warn in `tests/test_commands.py`; docs in `docs/config.md` (`connect_check`), `docs/providers.md`, `docs/commands.md`, `docs/testing.md`
