...
- `ToolRegistry.execute()` drops arguments not declared in a tool's schema — model-supplied extras (e.g. a hallucinated `depth` on `list_dir`) no longer error the tool; worktree semantics documented (worktree = launch directory / `--path`; launching from `~` makes home the worktree, so subdirectories don't escalate)
