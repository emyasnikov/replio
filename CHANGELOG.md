...
- Delegation workflow fixes - `delegate` now defaults to `allow` (runs without a confirm; `DEFAULT_CONFIG.tool_permission.delegate` and the per-persona resolver default; a persona can still set `delegate: "ask"`), so `/tool delegate` no longer prompts. Sub-agent sessions link to their parent: `Session` gains `parent_id` and `sub_sessions` (persisted), `run_subagent` records both (`/session preview` shows `parent`/`sub-sessions`, `/session list` annotates `delegate_*` with `(child of ...)`). A sub-agent that finishes with no prose now returns a log-summary result instead of `(no content)` - files written (`write_file`), tool round counts, and the last `bash` line are read back from its session - keeping the delegate result informative for the lead model and the REPL. Direct `/tool` calls no longer double-print the delegate result (`ToolRegistry.execute()`/`Engine._run_tool` take an `echo` flag injected as a hidden `_echo` kwarg; `/tool` passes `echo=False`). Docs synced (`config.md`, `personas.md`, `swarm.md`, `commands.md`, `security.md`, `session.md`, `testing.md`, `AGENTS.md`)
