# Replio Documentation

Detailed reference for Replio. For an overview, features, and quick start, see the [README](../README.md).

- [API endpoints](api.md) — `replio serve` HTTP JSON API
- [Configuration](config.md) — config schema and keys
- [Commands & CLI](commands.md) — slash commands and headless CLI flags
- [Plugins](plugins.md) — plugins: bundled, layout, manifest, management
- [Agent fleets](fleet.md) — single-purpose agent fleets, one scoped process per agent, deployment
- [Deployment](deploy.md) — Docker, systemd, and launchd configuration
- [Comparisons](comparison/) — Replio vs. OpenClaw, OpenCode, and Pi, plus the broader landscape
