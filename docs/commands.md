...
| `/focus`                |                | Show and switch the focused agent run. No args shows the current run, the run tree (with session codes), and the call log. Attach by `<#code>`, `<id|#id>`, `<role>`, or `session:<name>`, and navigate with `parent`, `child [n]`, `sibling [n]`, `next`, `prev`, or `back`. See [architecture.md](architecture.md#focus) |
