...
- `grant_permission` - optional ceiling on the categories this role may hand down to sub-agents. Defaults to the role's own `tool_permission`, so it never widens delegation unless set explicitly. See [Delegation and permissions](#delegation-and-permissions).
