...
The `ask` tool is the explicit human-in-the-loop channel: it pauses the run and poses a question to the operator at the terminal (`target='human'`) or to the lead agent that delegated the run (`target='lead'`, a bounded model consultation). It is the only tool that deliberately talks to a person. In headless mode there is no terminal and a root engine has no lead, so `ask` returns an error result and the run continues without pausing. In unattended mode (`unattended: true`, `replio --unattended`, `/unattended`) the terminal UI is dropped at the root and never propagated to sub-agents, so no ask at any depth reads stdin: confirms auto-deny and human asks park as pending requests in `.replio/asks.json` (returned to the agent as `[parked] Ask #<id>`, answered later via `/asks` or the serve API). Agents that must not ask can be stripped of the tool with `tools.deny: ["ask"]`. The question and answer are persisted in the asking session's log, so what was asked and decided stays on the record.
...
| Delegation | Per-type per-invocation resolution (`delegate: allow` default, set `ask` per type, non-registry types denied), sub-agents auto-deny `ask` and share only the caller's carve, own `sub_*` session logs |
