...
| `test_plugins.py` | Plugin manager: manifest compat ranges, discovery precedence, registration hooks (tools/providers/commands/services/personas/teams/skills + hook-failure status), install/update/uninstall, `replio plugins test` (+ `load_plugin_test_suite`) |
