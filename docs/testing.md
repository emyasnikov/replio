...
| `test_bundled_plugins.py` | Bundled plugin discovery, tool registration, search service, bundled update/uninstall blocking, default-config plugin set, provider registration via the hook |
...
| `test_ollama_provider.py` | Streaming provider: fragmented tool-call reassembly, thinking events (`reasoning_content` and `reasoning` keys), payload construction. Lives in the `replio-core-ollama` plugin suite (see the bundled-plugin suites note below) |
