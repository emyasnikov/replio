...
| `test_jobs.py` | Job model + registry (round-trip, runnable gate, corrupt file tolerance), cron parser (steps/ranges/lists/dom/dow/the restrictive day rule, leap day), `next_run`/`compute_next_run`/`parse_dt`, scheduler run/tick under a mocked engine (verified/failed, retries with backoff, per-attempt history, unknown persona, one-shot `at`, approval gates), `replio jobs` CLI (add/approve/list, auto-approval, bad cron, duplicates, run exit codes) |
