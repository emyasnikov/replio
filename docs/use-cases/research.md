...
- **Grounded answers with sources** - `web_search` / `web_fetch` (alias `open`) pull current literature and surface the sources. File tools keep the model answering from your own corpus (papers, notes, lab exports) rather than from memory.
...
- **Corpus queries** - point an agent at a folder of papers or notes (`replio serve --path ~/papers`) and ask comparative questions, with `grep`/`file_read` grounding every answer in the actual text.
...
- **Writing support** - draft methods, related-work, and appendix text from the corpus with citations attached, then `file_write` for review.
