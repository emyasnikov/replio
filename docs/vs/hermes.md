...
| Delegation | Yes - `delegate` tool with type sub-agents (in-process, own session logs). Fleet/swarm orchestration and auditors on the roadmap | Isolated subagents, background process management, PTY mode for interactive CLIs |
