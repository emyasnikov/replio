...
- Prefer search and point tools over dump-everything tools. An agent wastes its limited context reading irrelevant results. `grep` returns only matching `file:line: text` lines. `glob` locates a path before it is read. `read_file` reports `N lines, M chars` in its header so the agent can probe a file's size with `limit=0` before committing to a read.
- Consolidate operations that are usually chained. `write_file` folds create, overwrite, and append into one call behind a `mode` enum. `delegate` runs a whole task under a persona as a single call. A consolidated tool offloads multi-step reasoning from the model's context into the tool itself.
...
- Avoid near-duplicate tools. Two tools that fetch a web page (`open` and `fetch_page`) confused agents until they were merged. A single fetch tool takes `url` or a search-result `id`. If two tools overlap, merge them or delete one.

## Naming and namespacing

Tool names are how the model selects behavior, so they should be distinct, purposeful, and grouped by domain.

- Give every tool a clear, distinct purpose and a name that states it. `web_search`, `fetch_page`, `read_file`, `list_dir`, `write_file`, `glob`, `grep`, `run_command` all read as one distinct action each.
- Namespace by domain when a surface grows. The bundled plugins already do this by module (`replio-core-websearch` exposes `web_search`, `fetch_page`), and MCP tools use the `mcp_` prefix. Prefixing (`web_`, `fs_`, `mcp_`) helps the model pick the right tool when many are loaded.
- Advertise canonical names only, and absorb model dialect with `aliases`. The provider schema shows `run_command`, not `bash` or `exec`, and `web_search`, not `search`. Aliases resolve at call time, so a model that still says `search` or `find` gets the real tool anyway. Advertising both spellings doubles the tool list in every request for no benefit.
...
- Resolve and report natural identifiers. `read_file` and `write_file` resolve absolute paths. `grep` returns `file:line: text` so the model can act on the location. No raw UUIDs or opaque handles appear in Replio's tools.
- Report sizes and boundaries so the agent can decide. `read_file` puts the total line and char count in its header and marks partial windows with `(showing a-b)`. `fetch_page` reports `[offset N of M chars]` when content continues.
...
- Page large reads. `read_file(path, offset=1, limit=200)` then `offset=201` walks a file window by window. `fetch_page` accepts the `offset` its marker reports. Both let the agent pull only what it needs.
...
- State the purpose, the output format, and the alternatives in the description. `read_file` explains the header, the `limit=0` size probe, and paging. `glob` shows example patterns (`"**/*.py"`, `"src/**/chat.py"`). `grep` states its `file:line:` result format. `run_command` documents stdout, stderr, and exit code.
...
- Enforce the contract with types, enums, and `required`. The registry passes only declared, non-null arguments to the handler, so a strict schema is the model's only input surface. Use an enum for closed sets (write_file's `mode`), and always describe each parameter.
...
- Start with `Error:` and a one-line actionable message. Replio's filesystem tools set the pattern: `Error: X is a directory (use list_dir instead)`, `Error: X is not a directory (use read_file instead)`, `Error: file not found: X`. Each suggests the correct tool.
...
- Read the session logs for misbehavior. A session that shows wrong-tool selection (calling `open` when it meant `fetch_page`), repeated parameter errors, or the same search re-run means the description, naming, or schema is unclear.
